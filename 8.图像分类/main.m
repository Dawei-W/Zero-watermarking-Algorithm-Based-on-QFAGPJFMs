...
if MODE==11
   for i=1:1:OBJ
        I=imread(['Dateset\coil-20\training set\obj',num2str(i),'__0','.png']);
       [m,n] = size(I);
        [r,o]=ro(m,n);
        pz1=r>1;
        I(pz1)=0;%%%%单位圆化
        trainLabels(i,1)=i;     %%%%%训练号
        trainData(i, :) = AGPJFM_D(double(I), K, alpha, h)';  %%%%每一行是矩分解系数构成的训练集数据
   end
else
for i=1:1:OBJ
...
%% testing
testLabels =zeros(OBJ*37,1);
testData=zeros(OBJ*37,NoF);   %%%%%一列数据是一个图像在旋转不同角度下的测试数据

if MODE==11 
 for i=1:1:OBJ
    for j=0:1:36
        I=imread(['Dateset\coil-20\testing set\obj',num2str(i),'__',num2str(j),'.png']);
%         NI=imnoise(I,'gaussian',0,0.05);   
%         NI = imnoise(I, 'salt & pepper',VAR);
        NI = awgn(double(I), SNR,'measured');
        NII=uint8(abs(NI));
        imshow(NII);
        [m,n] = size(NII);
        [r,o]=ro(m,n);
...
%         imshow(uint8(abs(NI)));
        ...
        testData(k, :) = AGPJFM_D(NII, K, alpha, h)';   %%%%每一个特征值乘上图像像素构成测试集数据
    end
end
else
for i=1:1:OBJ
...
accuracy = length(find(label == testLabels))/(OBJ*37)*100;
UCCP(flg,1)=accuracy;
if mod(flg,2)==1
    disp(['	',num2str(SNR),'			',num2str(accuracy)]);
else
    disp(['	',num2str(SNR),'		    ',num2str(accuracy)]);
end
flg=flg+1;
